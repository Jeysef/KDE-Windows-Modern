...
{
    if (m_settings) connect(m_settings, &SystemTraySettings::configurationChanged, this, &BaseModel::onConfigurationChanged);
}
...
PlasmoidModel::PlasmoidModel(const QPointer<SystemTraySettings> &settings, const QPointer<PlasmoidRegistry> &plasmoidRegistry, QObject *parent)
    : BaseModel(settings, parent), m_plasmoidRegistry(plasmoidRegistry)
{
    ...
    for (const auto &info : appletMetaDataList) {
        if (!info.isValid() || info.value(u"X-Plasma-NotificationAreaCategory").isEmpty()) continue;
        appendRow(info);
    }
}
...
QVariant PlasmoidModel::data(const QModelIndex &index, int role) const {
    ...
    const auto &meta = item.pluginMetaData;
    auto *applet = item.applet;
    if (role <= Qt::UserRole) {
        switch (role) {
        case Qt::DisplayRole: return meta.name();
        case Qt::DecorationRole: { QIcon icon = QIcon::fromTheme(applet ? applet->icon() : QString(), QIcon::fromTheme(meta.iconName())); return icon.isNull() ? QVariant() : icon; }
        default: return {};
        }
    }
    if (role < static_cast<int>(Role::Applet)) {
        Plasma::Types::ItemStatus status = applet ? applet->status() : Plasma::Types::ItemStatus::UnknownStatus;
        switch (static_cast<BaseRole>(role)) {
        case BaseRole::ItemType: return QStringLiteral("Plasmoid");
        case BaseRole::ItemId: return meta.pluginId();
        case BaseRole::CanRender: return applet != nullptr;
        case BaseRole::Category: return plasmoidCategoryForMetadata(meta);
        case BaseRole::Status: return status;
        case BaseRole::EffectiveStatus: return calculateEffectiveStatus(applet != nullptr, status, meta.pluginId());
        default: return {};
        }
    }
    switch (static_cast<Role>(role)) {
    case Role::Applet: return applet ? QVariant::fromValue(PlasmaQuick::AppletQuickItem::itemForApplet(applet)) : QVariant();
    case Role::HasApplet: return applet != nullptr;
    default: return {};
    }
}

int PlasmoidModel::rowCount(const QModelIndex &parent) const { return parent.isValid() ? 0 : m_items.size(); }
...
void PlasmoidModel::addApplet(Plasma::Applet *applet) {
    auto meta = applet->pluginMetaData();
    int idx = indexOfPluginId(meta.pluginId());
    if (idx < 0) { idx = rowCount(); appendRow(meta); }
...
void PlasmoidModel::removeApplet(Plasma::Applet *applet) {
    int idx = indexOfPluginId(applet->pluginMetaData().pluginId());
    if (idx >= 0) { m_items[idx].applet = nullptr; Q_EMIT dataChanged(index(idx, 0), index(idx, 0)); applet->disconnect(this); }
}

void PlasmoidModel::appendRow(const KPluginMetaData &pluginMetaData) {
    int idx = rowCount();
    beginInsertRows(QModelIndex(), idx, idx);
    m_items.append({pluginMetaData, nullptr});
    endInsertRows();
}
...
int PlasmoidModel::indexOfPluginId(const QString &pluginId) const {
    for (int i = 0; i < rowCount(); i++)
        if (m_items[i].pluginMetaData.pluginId() == pluginId) return i;
    return -1;
}

StatusNotifierModel::StatusNotifierModel(QObject *parent) : BaseModel(nullptr, parent) { init(); }
StatusNotifierModel::StatusNotifierModel(QPointer<SystemTraySettings> settings, QObject *parent) : BaseModel(settings, parent) { init(); }

static Plasma::Types::ItemStatus extractStatus(const StatusNotifierItemSource *sniData) {
    QString status = sniData->status();
    if (status == QLatin1String("Active")) return Plasma::Types::ItemStatus::ActiveStatus;
    else if (status == QLatin1String("NeedsAttention")) return Plasma::Types::ItemStatus::NeedsAttentionStatus;
    else if (status == QLatin1String("Passive")) return Plasma::Types::ItemStatus::PassiveStatus;
    else return Plasma::Types::ItemStatus::UnknownStatus;
}
...
static QString extractItemId(StatusNotifierItemSource *sniData) {
    ...
    if (itemId.startsWith(QLatin1String("dropbox-client-"))) return QLatin1String("dropbox-client-PID");
    if (itemId.startsWith(QLatin1String("chrome_status_icon_"))) {
        const QString processName = sniData->processName();
        if (!processName.isEmpty()) return itemId + QLatin1Char('@') + processName;
    }
    return itemId;
}
...
QVariant StatusNotifierModel::data(const QModelIndex &index, int role) const {
    ...
    if (role <= Qt::UserRole) {
        switch (role) {
        case Qt::DisplayRole: return sniData->title();
        case Qt::DecorationRole: return sniData->iconName().isNull() ? extractIcon(sniData->icon()) : sniData->iconName();
        default: return {};
        }
    }
    ...
    if (role < static_cast<int>(Role::DataEngineSource)) {
        ...
        case BaseRole::ItemId: return itemId;
        case BaseRole::CanRender: return true;
        case BaseRole::Category: { QVariant cat = sniData->category(); return cat.isNull() ? QStringLiteral("UnknownCategory") : cat; }
        case BaseRole::Status: return extractStatus(sniData);
        case BaseRole::EffectiveStatus: return calculateEffectiveStatus(true, extractStatus(sniData), itemId);
        default: return {};
        }
    }
    switch (static_cast<Role>(role)) {
    case Role::DataEngineSource: return source;
    case Role::AttentionIcon: return extractIcon(sniData->attentionIcon());
    case Role::AttentionIconName: return sniData->attentionIconName();
    case Role::AttentionMovieName: return sniData->attentionMovieName();
    case Role::Category: return sniData->category();
    case Role::Icon: return extractIcon(sniData->icon());
    case Role::IconName: return sniData->iconName();
    case Role::IconThemePath: return sniData->iconThemePath();
    case Role::Id: return itemId;
    case Role::ItemIsMenu: return sniData->itemIsMenu();
    case Role::OverlayIconName: return sniData->overlayIconName();
    case Role::Status: return extractStatus(sniData);
    case Role::Title: return sniData->title();
    case Role::ToolTipSubTitle: return sniData->toolTipSubTitle();
    case Role::ToolTipTitle: return sniData->toolTipTitle();
    case Role::WindowId: return sniData->windowId();
    default: return {};
    }
}

int StatusNotifierModel::rowCount(const QModelIndex &parent) const { return parent.isValid() ? 0 : m_items.size(); }
...
void StatusNotifierModel::addSource(const QString &source) {
    ...
    connect(sni, &StatusNotifierItemSource::dataUpdated, this, [=, this]() { dataUpdated(source); });
...
void StatusNotifierModel::removeSource(const QString &source) {
    ...
    if (idx >= 0) { beginRemoveRows(QModelIndex(), idx, idx); m_items.removeAt(idx); endRemoveRows(); }
}

void StatusNotifierModel::dataUpdated(const QString &sourceName) {
    int idx = indexOfSource(sourceName);
    if (idx >= 0) Q_EMIT dataChanged(index(idx, 0), index(idx, 0));
}

int StatusNotifierModel::indexOfSource(const QString &source) const {
    for (int i = 0; i < rowCount(); i++)
        if (m_items[i] == source) return i;
    return -1;
}
...
void SystemTrayModel::addSourceModel(QAbstractItemModel *sourceModel) {
    ...
    while (it.hasNext()) { it.next(); if (!m_roleNames.contains(it.key())) m_roleNames.insert(it.key(), it.value()); }
